...
fn main() -> Result<()> {
    ...
    let report = if args.iai {
        ...
        if !criterion_dir.exists() {
            anyhow::bail!("Criterion results directory not found at {}", criterion_dir.display());
        }
...
fn generate_criterion_report(criterion_dir: &PathBuf) -> Result<String> {
    ...
    writeln!(&mut report, "Full benchmark data is stored in `target/criterion/` ({} groups)", groups.len())
        .map_err(|err| anyhow::anyhow!("Failed to write to string: {err}"))?;
...
fn generate_iai_report(output: &Output) -> Result<String> {
    ...
    report.push_str("## Summary\n\n");
    report.push_str("IAI benchmarks use Cachegrind to provide precise, deterministic measurements.\n\n");
