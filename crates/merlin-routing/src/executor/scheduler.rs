...
impl ConflictAwareTaskGraph {
    ...
    fn conflicts_with_running(&self, task: &Task, running: &HashSet<TaskId>) -> bool {
        task.context_needs.required_files.iter().any(|file| {
...
mod tests {
    ...
    fn test_conflict_detection() {
        ...
        assert_eq!(
            ready.len(),
            2,
            "Both tasks ready when nothing running - executor picks one"
        );
        ...
        assert_eq!(
            ready_after.len(),
            0,
            "Task B should be blocked due to file conflict with running Task A"
        );
    }
