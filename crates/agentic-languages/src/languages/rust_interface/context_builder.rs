...
pub struct ContextBuilder<'analysis> {
    ...
    /// The Rust language provider used for file/id resolution
    provider: &'analysis RustLanguageProvider,
}

impl<'analysis> ContextBuilder<'analysis> {
    /// Create a new context builder bound to an analysis session and provider.
    #[must_use]
    ...
    fn resolve_import(&self, import_path: &str, current_file: &Path) -> Option<PathBuf> {
        ...
        if parts[0] == "crate" {
            let project_src = self.provider.project_root.join("src");
    ...
    #[allow(clippy::only_used_in_recursion, reason = "Recursive descent through module tree requires passing the parts slice")]
