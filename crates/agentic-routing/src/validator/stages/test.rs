...
impl ValidationStage for TestValidationStage {
    async fn validate(&self, _response: &agentic_core::Response, task: &Task) -> Result<StageResult> {
        if !task.requires_build_check() {
            return Ok(StageResult {
                stage: StageType::Test,
                passed: true,
                duration_ms: 0,
                details: "Test check skipped (no files modified)".to_string(),
                score: 1.0,
            });
        }
        ...
        let Some(workspace) = &self.workspace else {
            return Ok(StageResult {
                ...
                passed: true,
                duration_ms: 0,
                details: "Test check skipped (no workspace)".to_string(),
                score: 1.0,
            });
        };
...
mod tests {
    ...
    async fn test_quick_check() {
        ...
        let good_response = agentic_core::Response {
            text: "test result: ok. 5 passed".to_string(),
            confidence: 1.0,
            tokens_used: agentic_core::TokenUsage::default(),
            provider: "test".to_string(),
            latency_ms: 0,
        ...
        let bad_response = agentic_core::Response {
            text: "test result: FAILED. 2 passed; 3 failed".to_string(),
            confidence: 1.0,
            tokens_used: agentic_core::TokenUsage::default(),
            provider: "test".to_string(),
            latency_ms: 0,
