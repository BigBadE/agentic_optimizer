...
# Usage:
#   ./scripts/commit.sh [--no-cloud] [--ollama]
#
# Flags:
...
cargo llvm-cov --no-report --ignore-filename-regex "test_repositories|benchmarks" --all-features --workspace --no-fail-fast nextest

# Run ignored env var tests serially with coverage
echo "[commit] Running ignored env var tests (serial)..."
cargo llvm-cov --no-report --ignore-filename-regex "test_repositories|benchmarks" -p merlin-context nextest -- --ignored

# Generate coverage report
echo "[commit] Generating coverage report (can be slow)..."
