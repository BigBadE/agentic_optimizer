...
use super::types::{Action, Complexity, QueryIntent, Scope};
...
impl QueryAnalyzer {
    ...
    fn detect_scope(query: &str) -> Scope {
        ...
        const MODULE_KEYWORDS: &[&str] = &["module", "package", "folder", "directory", "related"];

        if CODEBASE_KEYWORDS
            .iter()
            .any(|keyword| query.contains(keyword))
        {
            Scope::Codebase
        } else if MODULE_KEYWORDS
            .iter()
            .any(|keyword| query.contains(keyword))
        {
            Scope::Module
...
mod tests {
    ...
    fn test_extract_entities() {
        ...
        assert!(
            intent
                .entities
                .contains(&"UserService::find_by_email".to_string())
        );
    }
