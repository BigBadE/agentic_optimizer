...
merlin-routing/
├── src/
│   ├── analyzer/           # Task analysis and decomposition
│   │   ├── intent.rs       # Intent extraction (Action, Scope detection)
│   │   ├── complexity.rs   # Multi-factor complexity estimation
│   │   ├── decompose.rs    # Smart task splitting
│   │   └── local.rs        # LocalTaskAnalyzer (no LLM required)
│   ├── router/             # Model tier selection
│   │   ├── strategy.rs     # RoutingStrategy trait
│   │   ├── strategies/     # Individual routing strategies
│   │   │   ├── complexity.rs  # Complexity-based routing
│   │   │   ├── cost.rs        # Cost optimization
│   │   │   ├── quality.rs     # Quality-critical routing
│   │   │   └── context.rs     # Long-context routing
│   │   └── tiers.rs        # StrategyRouter orchestrator
│   ├── executor/           # Parallel execution & isolation
│   │   ├── graph.rs        # Task dependency graph
│   │   ├── pool.rs         # Parallel executor
│   │   ├── state.rs        # Workspace state management
│   │   ├── isolation.rs    # File-level locking (RAII guards)
│   │   ├── transaction.rs  # Transactional workspaces
│   │   ├── build_isolation.rs  # Isolated build environments
│   │   └── scheduler.rs    # Conflict-aware scheduling
│   ├── validator/          # Multi-stage validation
│   │   ├── pipeline.rs     # ValidationPipeline orchestrator
│   │   └── stages/         # Individual validation stages
│   │       ├── syntax.rs   # Heuristic syntax checking
│   │       ├── build.rs    # Cargo check validation
│   │       ├── test.rs     # Cargo test execution
│   │       └── lint.rs     # Clippy linting
│   ├── ui/                 # TUI interface
│   │   ├── mod.rs          # TuiApp with ratatui
...
│   └── lib.rs              # Public API
├── examples/
│   └── basic_routing.rs    # Complete usage example
└── tests/
...
- **Analyzer** (18 tests)
  ...
  - LocalTaskAnalyzer: 4 tests
  
- **Router** (13 tests)
  - Complexity strategy: 3 tests
  - Cost optimization: 3 tests
  - Quality critical: 3 tests
  - Long context: 3 tests
  - StrategyRouter: 1 test
...
- **Executor** (12 tests)
  ...
  - Executor pool: 1 test
  
- **Validator** (11 tests)
  - Pipeline: 2 tests
  - Syntax stage: 3 tests
  - Build stage: 2 tests
  - Test stage: 2 tests
  - Lint stage: 2 tests
  
- **Config** (2 tests)
  - Default config: 1 test
...
See `tests/integration_tests.rs` for comprehensive test scenarios including:
