...
pub trait ValidationStage: Send + Sync {
    async fn validate(&self, response: &merlin_core::Response, task: &Task) -> Result<StageResult>;
...
mod tests {
    ...
    impl ValidationStage for MockStage {
        async fn validate(&self, _response: &merlin_core::Response, _task: &Task) -> Result<StageResult> {
            Ok(StageResult {
        ...
        async fn quick_check(&self, _response: &merlin_core::Response) -> Result<bool> {
            Ok(self.should_pass)
        }
