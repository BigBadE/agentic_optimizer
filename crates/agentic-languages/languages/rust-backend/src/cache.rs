...
impl WorkspaceCache {
    ...
    pub fn save(&self, project_root: &Path) -> Result<()> {
        ...
        if let Some(parent) = cache_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|error| agentic_core::Error::Other(format!("Failed to create cache directory: {error}")))?;
        }

        let file = File::create(&cache_path)
            .map_err(|error| agentic_core::Error::Other(format!("Failed to create cache file: {error}")))?;
        
        let writer = BufWriter::new(file);
        bincode::serialize_into(writer, self)
            .map_err(|error| agentic_core::Error::Other(format!("Failed to serialize cache: {error}")))?;
    ...
    pub fn load(project_root: &Path) -> Result<Self> {
        ...
        if !cache_path.exists() {
            return Err(agentic_core::Error::Other("Cache file does not exist".into()));
        }

        let file = File::open(&cache_path)
            .map_err(|error| agentic_core::Error::Other(format!("Failed to open cache file: {error}")))?;
        
        let reader = BufReader::new(file);
        let cache: Self = bincode::deserialize_from(reader)
            .map_err(|error| agentic_core::Error::Other(format!("Failed to deserialize cache: {error}")))?;
    ...
    pub fn clear(project_root: &Path) -> Result<()> {
        ...
        if cache_path.exists() {
            fs::remove_file(&cache_path)
                .map_err(|error| agentic_core::Error::Other(format!("Failed to delete cache: {error}")))?;
